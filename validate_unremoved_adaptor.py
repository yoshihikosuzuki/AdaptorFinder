...
def validate_candidate(candidate):
    ...
    header, match_length, forward = candidate
    ...
    read_len, identity, ovlp_len, fstart, fend, rstart, rend = extract_alignment_info(commands.getoutput("~/work2/metaFALCON/etc/lalign36 -3K1 -f -4 -g -4 " + f_name + " " + r_name))
...
if __name__ == "__main__":
    ...
    import argparse
    parser = argparse.ArgumentParser(description="a multi-processes adaptor unremoved subreads finder")
    ...
    #suffix = re.search(r'.*\.(.*?)\..*?', sys.argv[1]).groups()[0]
    #out = open("result." + suffix + "." + sys.argv[3], 'w')
    candidates = []
    with open(candidate_file, 'r') as f:
        ...
        for line in f:
            ...
            candidates.append( (header, match_length, seqs[header]) )
            #count += 1
            #if count == 10:
            #    break
    #out.close()
